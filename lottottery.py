...
"获取网络数据"
"sNo: 最近期数"
def getData(sNo, conn):
    url = "http://www.cwl.gov.cn/cwl_admin/kjxx/findDrawNotice?name=ssq&issueCount=" + str(sNo)
    headers = {"Host": "www.cwl.gov.cn", "Referer": "http://www.cwl.gov.cn/kjxx/ssq/kjgg/", \
    ...
    col = 0
    for item in result:
        arrSeral.append(item['code'])
        arrDate.append(item['date'])
        blueBalls[row] = int(item['blue'])
        reds_str = item['red']
        reds = reds_str.split(',')
        for col in range(6):
            redBalls[row][col] = int(reds[col])
#        print(redBalls[row])
        row = row + 1
        values = (item['code'], item['blue'], item['red'], item['date'])
...
def sumOffset(ws):
    ...
    for i in range(0, 6):
        ...
        red_str = red_str + ' %02d' % redBalls[startSeral][i]
    cells = ['' for row in range(21)]
    cells[0] = arrDate[startSeral]
    cells[1] = arrSeral[startSeral]
    cells[2] = red_str + ' + %02d' % blueBalls[startSeral]
    cells[3] = str(sum)
...
def createXlsx():
    ...
    ws1 = wb.create_sheet(index=1,title=u'奇偶数表')

    ws2 = wb.create_sheet(index=2,title=u'大小数表')

    ws3 = wb.create_sheet(index=3,title = u'和值偏差表')
    ws3.title = u'和值偏差表'
    ws3.append(['日期', '期数', '中奖号码', '和值'])
    applyXlsFont(ws3)

    ws4 = wb.create_sheet(index=4, title = u'区间表')
    ws4.title = u'区间表'
    ws4.append(['日期', '期数', '1-5', '6-10', '11-15', '16-20', '21-25', '26-30', '31-33'])
    applyXlsFont(ws4)

    ws5 = wb.create_sheet(index=5, title = u'百分比表')
    ws5.title = u'百分比表'
    ws5.append(['日期', '期数', '热号', '冷号', '温号'])
    applyXlsFont(ws5)

    ws6 = wb.create_sheet(index=6, title = u'遗漏表')
    ws6.title = u'遗漏表'
    ws6.append(['日期', '期数', '中奖号码', '遗漏情况', '遗漏少于10次的个数', '总计', '平均'])
    applyXlsFont(ws6)

    ws7 = wb.create_sheet(index=7,title = u'遗漏数字偏差表')
    ws7.title = u'遗漏数字偏差表'

    ws8 = wb.create_sheet(index=8,title = u'中期表')
    ws8.title = u'中期表'
